...
def _conv(x, filter_size, out_channel, strides, pad='SAME', input_q=None, output_q=None, name='conv'):
    ...
    with tf_variable_scope(name):
        # Main operation: conv2d
        with tf.device('/CPU:0'):
            kernel = tf_get_variable('kernel', [filter_size, filter_size, in_shape[3], out_channel],
                            tf.float32, initializer=tf.random_normal_initializer(
                                stddev=np.sqrt(2.0/filter_size/filter_size/out_channel)))
        if kernel not in tf_get_collection(WEIGHT_DECAY_KEY):
            tf_add_to_collection(WEIGHT_DECAY_KEY, kernel)
...
def _fc(x, out_dim, input_q=None, output_q=None, name='fc'):
    ...
    with tf_variable_scope(name):
        # Main operation: fc
        with tf.device('/CPU:0'):
            w = tf_get_variable('weights', [x.get_shape()[1], out_dim],
                            tf.float32, initializer=tf.random_normal_initializer(
                                stddev=np.sqrt(1.0/out_dim)))
            b = tf_get_variable('biases', [out_dim], tf.float32,
                                initializer=tf.constant_initializer(0.0))
        if w not in tf_get_collection(WEIGHT_DECAY_KEY):
            tf_add_to_collection(WEIGHT_DECAY_KEY, w)
...
def _get_split_q(ngroups, dim, name='split', l2_loss=False):
    with tf_variable_scope(name):
        # alpha = tf_get_variable('alpha', shape=[ngroups, dim], dtype=tf.float32,
        ...
        with tf.device('/CPU:0'):
            q = tf_get_variable('q', shape=[ngroups, dim], dtype=tf.float32,
                                # initializer=tf.constant_initializer(1.0/ngroups))
                                initializer=tf.constant_initializer(init_val))
        if l2_loss:
            if q not in tf_get_collection(WEIGHT_DECAY_KEY):
                tf_add_to_collection(WEIGHT_DECAY_KEY, q*2.236)
...
def _add_split_loss(w, input_q, output_q):
    ...
    T_list = []
    U_list = []
    if input_q not in tf_get_collection('OVERLAP_LOSS_WEIGHTS'):
        tf_add_to_collection('OVERLAP_LOSS_WEIGHTS', input_q)
    ...
    if output_q not in tf_get_collection('OVERLAP_LOSS_WEIGHTS'):
        print('\t\tAdd overlap & split loss for %s' % output_q.name)
        tf_add_to_collection('OVERLAP_LOSS_WEIGHTS', output_q)
        for i in range(ngroups):
            for j in range(ngroups):
                if i == j:
                    continue
    ...
    if T_list:
        tf_add_to_collection('OVERLAP_LOSS', tf.add_n(T_list))
    if U_list:
        tf_add_to_collection('UNIFORM_LOSS', tf.add_n(U_list))
...
def _bn(x, is_train, global_step=None, name='bn'):
    ...
    with tf_variable_scope(name):
        ...
        with tf.device('/CPU:0'):
            mu = tf_get_variable('mu', batch_mean.get_shape(), tf.float32,
                            initializer=tf.zeros_initializer(), trainable=False)
            sigma = tf_get_variable('sigma', batch_var.get_shape(), tf.float32,
                            initializer=tf.ones_initializer(), trainable=False)
            beta = tf_get_variable('beta', batch_mean.get_shape(), tf.float32,
                            initializer=tf.zeros_initializer())
            gamma = tf_get_variable('gamma', batch_var.get_shape(), tf.float32,
